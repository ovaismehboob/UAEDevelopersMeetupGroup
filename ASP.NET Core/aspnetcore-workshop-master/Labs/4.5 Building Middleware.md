...
1. Start with the application you created in [Lab 2](2. Introduction to ASP.NET Core.md), or just create a new empty ASP.NET Coe application
2. Open `Startup.cs`
...
8. Run the application again and see that the middleware is now running as a class
...
5. Run the application again and see the default culture when no query string is specified matches the one configured.

## Read request culture configuration from a file
1. Add a constructor to the application's `Startup.cs`
2. Create a new `Configuration` object in the constructor and assign it to a new private class field `IConfiguration _configuration`
...
8. Run the application and the default culture should be set from the configuration file.
...
5. In `Startup.cs` add a `ConfigureServices(ISeviceCollection services)` method and add a line that configures the culture using the `services.Configure<RequestCultureOptions>` method:
